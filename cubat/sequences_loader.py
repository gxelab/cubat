...
def sequences_to_codonframe(file_path, codon_table=1, exclude_qa_problems=False, display_warnings=False):
    """
    ...
    参数:
    - file_path (str): 使用read_sequences(file_path)读取序列。
    ...
    print("\n\n\033[32mSequences has been loaded.\033[0m\n\n")
    codonframe = pd.DataFrame(data)
    codonframe = codonframe.set_index("ID")
    # TODO 加一个类似的dataframe看每条序列的问题，可以放在qa函数里边。
    return codonframe
